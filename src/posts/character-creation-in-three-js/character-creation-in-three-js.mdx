...
summary: Using Morph Targets to create user generated characters.
date: "2021-08-06T22:47:52.488Z"
...
import { CanvasProvider2 } from "./CanvasProvider.js";
...
I would export each part with its variations in one file. So the head and it’s variations resided in `head.gltf`. The variations of the head were named with this convention - `head_1`, `head_2`…`head_n`.
...
Another issue was to generate another random character on the fly, the file would have to be loaded again or the scene to be saved in memory, then the process repeated.
...
To use Morph Targets, first you need to export your model with them in its data. In Blender, this is as simple as creating some ShapeKeys and exporting the model with them enabled.

Next, within ThreeJS, once the model has been imported, the Mesh will contain a couple of properties

- `morphTargetDictionary`: This is a dictionary where the keys are the names of the ShapeKeys and the values are the Indices. Indices into what? Well…
- `morphTargetInfluences`: Indices into this array. This array holds “factor” values for each ShapeKey, just like the slider in blender.
...
This means many things, one of which is that we can connect that value up to a slider. Then have sliders control our variations. This way we can make a rudimentary character creation tool.

Also, since each influence can range from 0 to 1, and there are almost an infinite number of values in between, we can get an almost infinite number of combinations!

We can also drive multiple Shape Keys at once with one value. I asked the artist to name the keys he wants to be driven together with the same prefix. This way no element is misaligned or clips another as different parts morph.
...
The new models were created by a very talned artist @Mox. He carefully modled eight shapekeys into each mesh. Here is just the head he modled.

![Models](../../images/character-creation-in-three-js/models.gif)

The models were exported in the GLTF file format using Blender's glTF 2.0 plugin.

# The code

With all that context our of the way, let's dive into the code. This section will move a bit quickly. This is not meant to be a "beginers guide".

## Loading Models

The models were loaded in using ThreeJS's [`GLTFLoader`](https://threejs.org/docs/index.html?q=gltf#examples/en/loaders/GLTFLoader) class.
...
huh...what gives? Did I export the model incorrectly? Nope! the gltf scene object is not our mesh. We need to traverse the scene graph of the gltf file and find our object.

Of course, we can find our object by name using [`Object3D.getObjectByName`](https://threejs.org/docs/#api/en/core/Object3D.getObjectByName) but I havent named my mesh when I exported it so im going to do it the old fassoned way.
...
A little ugly, I know, but now we can be sure that `head` will be available after the model loads.

### Storing them

Now that we know where they are, we can simply randomize them right where the `console.log` is. But that would mean that we can only get a new variation when the model is loaded again.

> Remember, all of this is within the `onLoad` callback of the `GLTFLoader.load` function.

Instead, we can store the targets and use them later. THis was we can also have extra logic to control targets of the same name together. Here is how I will be storing them

```ts
type morphTarget = {
  index: child.morphTargetDictionary[morphTargetName];
  child: child;
};
...
This way, all targets of the same name are stored together with a refrence to their currosponding meshes and the index into the meshes `morphTargetInfluences`. Here is what it looks like in code
...
We now have a nice object with all the information we need play with the morph targets at will.

## GUI

Let's create some sliders that will help us change look of our model. I will use the library [dat.GUI](https://github.com/dataarts/dat.gui) created by the creator of ThreeJS = MrDoob, to be used with ThreeJS.

I will loop through all our unique morph targets and create a slider for each one. Since the influence value should be between 0 and 1, I will set the `min` and `max` of the slider to those values.
...
The final step of the process and what we initially set out to do is to randomize the weights so that a random character is created on every click of a button. To do so, we simply loop over our `morphTargets` object and assign a random weight to each `morphTargetInfluence`.
...
I will add this function as a button within the GUI. For this part I will also group the sliders into one folder.
...
You can hook this concept up to a loop and generate an array of random faces like I did for the thumbnail of this article. Infact you can checkout [a the demo here.](https://farazzshaikh.github.io/experiments/?src=/3D/Randomized%20Characters/index.html). You can also find the code for the demo on GitHub here
